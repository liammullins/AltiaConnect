...
void HttpRequest::returnHeader(QString resultHeader)
{
    emit resultHeader;
}
...
void HttpRequest::readHeader(QTcpSocket* socket)
{
    ...
    if (colon>0)
    {
        ...
        returnHeader(QString("%s: %s").arg(currentHeader.data(),value.data()));

            //connectionManager->addIPToTable( QString("%s: %s").arg(currentHeader.data(),value.data()),"connected...." );
